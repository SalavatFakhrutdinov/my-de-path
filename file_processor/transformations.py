...
def normalize_name(user: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if original_name != transformed_name['name']:
        logger.debug(f"Приведенное имя: '{original_name}' -> '{transformed_name['name']}'")
...
def add_age_group(user: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug(f"Добавлена возрастная группа '{transformed_age['age_group']} для возраста {age} лет")
