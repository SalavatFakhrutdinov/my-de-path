...
"""
    ...
    parser.add_argument(
        '--input', '-i',
        required=True,
        help='Введите путь к JSON файлу'
    )

    parser.add_argument(
        '--output', '-o',
        required=True,
        help='Вывод пути CSV файла'
    )
...
def print_summary(stats: dict, output_file: str) -> None:
    ...
    print(f"Совершеннолетних пользователей (>={stats.get('min_age', 18)}):"
...
def run_apllication(args: argparse.Namespace) -> int:
    ...
    if not processed_data:
        logger.warning("Не обнаружено совершеннолетних пользователей"
